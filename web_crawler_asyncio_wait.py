...
class Solution:
    ...
    async def crawl_helper(self, startUrl: str, htmlParser: "HtmlParser") -> List[str]:
        ...
        async with asyncio.TaskGroup() as tg:
            startUrl_future = tg.create_task(
                asyncio.to_thread(htmlParser.getUrls, startUrl)
            )
            ...
            while futures:
                ...
                for new_url in new_urls:
                    if (
                        startUrl_hostname == self.get_hostname(new_url)
                        and new_url not in visited
                    ):
                        visited.add(new_url)
                        new_future = tg.create_task(
                            asyncio.to_thread(htmlParser.getUrls, new_url)
                        )
