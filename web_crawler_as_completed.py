...
class Solution:
    def process_url(self, url):
        urls = set(
            u for u in self.htmlParser.getUrls(url) if u.startswith(self.hostname)
        )
    ...
    def crawl(self, startUrl, htmlParser):
        ...
        with ThreadPoolExecutor(
            max_workers=64
        ) as self.executor:  # or can do self.executor = ThreadPoolExecutor(max_workers=10) but would have to call self.executor.shutdown() at the end
